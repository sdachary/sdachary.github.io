...
- Static site deploy to GitHub Pages (`sdachary.github.io`) via Actions build
- Scan script generates project data from local repos
- Contact form posts to a Cloudflare Worker (only remaining CF dependency)

## Data Flow
Vite build → static assets → GitHub Pages CDN
...
- **portfolio-contact** — CF Worker (`portfolio-contact.sdachary.workers.dev`). Contact form endpoint; stays on Cloudflare (server-side feature).
...
## Integration Points
- GitHub Pages (hosting, deploy from `.github/workflows/deploy.yml`)
- Cloudflare Workers (alert-router, portfolio-contact — unchanged)
